  ...
  useEffect(() => {
    ...
    const onChunk = ({ fileId, chunkIndex, totalChunks, data: base64 }) => {
      ...
      try {
        ...
        console.error('[Filevo] chunk decode error', err)
        return
      }
    ...
    const onComplete = ({ fileId }) => {
      ...
      if (missing > 0) {
        updateTransfer(fileId, {
          status: 'error',
          errorMsg: `${missing} chunks missing`,
        })
        return
      }
      ...
      setTransfers(prev => {
        ...
        if (t) {
          a.download = t.fileName || 'fvo_file'
          a.href = url
    ...
    return () => {
      socketService.off('file_manifest', onManifest)
      socketService.off('file_chunk', onChunk)
      socketService.off('file_complete', onComplete)
    }
  }, [updateTransfer])
    ...
    <div style={card}>
      ...
      {transferList.length === 0 && (
        <div style={emptyState}>
          ...
          <div style={{ fontFamily: "'Syne', sans-serif", fontWeight: 600, color: '#374151', fontSize: 15, marginBottom: 8 }}>
            No incoming files yet
          </div>
          <div style={{ fontFamily: "'Space Mono', monospace", color: '#1f2937', fontSize: 10, lineHeight: 1.8, textAlign: 'center' }}>
...
function TransferRow({ transfer: t }) {
  ...
  const elapsed =
    t.doneAt && t.startedAt
      ? ((t.doneAt - t.startedAt) / 1000).toFixed(1) + 's'
      : null
    ...
    <div style={transferCard(isDone, isError)}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: '#4b5563', marginTop: 3 }}>
            {formatBytes(t.fileSize)}
            {elapsed && <span style={{ marginLeft: 8, color: '#10b981' }}>· {elapsed}</span>}
          </div>
        </div>
